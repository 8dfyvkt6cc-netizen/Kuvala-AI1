...
interface SessionData {
  ...
  tempContext?: string;
  analytics: { totalGens: number; lastUsed: string };
  preferences: { autoHashtags: boolean; toneStrictness: number };
}
...
async function decrLimit(userId: number): Promise<void> {
...
async function logUsageStats(userId: number, action: string, tokens: number) {
  await pool.query('INSERT INTO bot_usage (user_id, kind, input_tokens) VALUES ($1, $2, $3)', [userId, action, tokens]);
}
    ...
    try {
      ...
      for (const row of r.rows) {
        try {
          ...
          try {
            const userLang = await pool.query('SELECT ui_lang FROM bot_users WHERE id = $1', [row.user_id]).then(r => r.rows[0]?.ui_lang || 'ru');
            await bot.api.sendMessage(
              Number(row.user_id),
              STRINGS[userLang as Lang].sendChannelFail(msg),
            );
           } catch (e) { console.error("Бля, опять ошибка в планировщике:", e) }
...
export async function startBot(): Promise<void> {
  ...
  bot.on("message:text", async (ctx) => {
    ...
    if (mode === "ab_test") {
      await ctx.replyWithChatAction("typing");
      const out = await compose(PROMPT_A_B_TEST, text, 3000, { userId, kind: "ab_test" });
      const [a, b] = out.split("===");
    ...
    if (mode === "reels_topic") {
      ...
      await logGeneration(userId, "reels", text, out);
  ...
  void bot.start({
    onStart: (info) =>
      logger.info({ username: info.username }, "Бот погнал! 🚀"),
  });
}
...
async function getTopUsers() {
  const r = await pool.query('SELECT user_id, COUNT(*) as count FROM bot_generations GROUP BY user_id ORDER BY count DESC LIMIT 10');
  return r.rows;
}
