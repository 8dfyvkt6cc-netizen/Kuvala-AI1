...
async function scrapeContent(url: string): Promise<ScrapingResult> {
...
async function trackDetailedUsage(userId: number, action: string, tokens: number) {
...
const PROMPT_POST = `Ты — топовый копирайтер. Твоя задача — писать посты, которые невозможно отличить от написанных человеком.
