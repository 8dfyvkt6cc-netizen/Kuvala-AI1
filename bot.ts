...
const fmtDate = (d: Date) => d.toISOString().split('T')[0].split('-').reverse().join('.');
...
const FREE_QUOTA = +(process.env["FREE_QUOTA"] ?? 10);
...
const TEXT_MODEL = process.env["BOT_TEXT_MODEL"] ?? "claude-sonnet-4-5";
const IMAGE_MODEL = process.env["BOT_IMAGE_MODEL"] ?? "imagen-3.0-generate-001";
...
type Mode =
  ...
  | "schedule_channel"
  | "schedule_text"
  | "schedule_when" | "reels_topic" | "context_input";
...
const initDb = async () => {
  await pool.query(`
    ...
    ALTER TABLE bot_users ADD COLUMN generations_used INT NOT NULL DEFAULT 0;
    ALTER TABLE bot_users ADD COLUMN subscription_until TIMESTAMPTZ;
    ALTER TABLE bot_users ADD COLUMN plan TEXT;
    ALTER TABLE bot_users ADD COLUMN ui_lang TEXT;
    ALTER TABLE bot_users ADD COLUMN captcha_passed BOOLEAN NOT NULL DEFAULT false;
    ALTER TABLE bot_users ADD COLUMN captcha_target TEXT;
...
const syncUser = async (ctx: BotContext) => {
  if (!ctx.from) return;
  await pool.query(
    `INSERT INTO bot_users (id, username, first_name, last_seen) VALUES ($1, $2, $3, now()) ON CONFLICT (id) DO UPDATE SET username = $2, first_name = $3, last_seen = now()`,
    [ctx.from.id, ctx.from.username ?? null, ctx.from.first_name ?? null],
  );
}

async function loadBrand(uid: number): Promise<BrandProfile> {
...
async function saveBrand(uid: number, brand: BrandProfile): Promise<void> {
  await pool.query(
...
async function logGeneration(
  ...
  await pool.query(
    ...
    [uid, kind, input.slice(0, 4000), output.slice(0, 8000)],
  );
}
...
async function getUserStatus(uid: number): Promise<UserStatus> {
  const res = await pool.query(
...
const useQuota =(uid: number): Promise<void> {
...
async function grantSubscription(
  ...
  const res = await pool.query(
    `INSERT INTO bot_users (id, subscription_until, plan)
     VALUES ($1, now() + interval '1 day' * $2::int, $3)
     ON CONFLICT (id) DO UPDATE SET
       subscription_until = GREATEST(COALESCE(subscription_until, now()), now()) + interval '1 day' * $2::int,
       plan = EXCLUDED.plan
     RETURNING subscription_until`,
    [uid, String(days), plan],
  );
  return new Date(res.rows[0].subscription_until);
}

async function revokeSubscription(uid: number): Promise<void> {
  await pool.query(
...
async function resetQuota(uid: number): Promise<void> {
  await pool.query(
...
async function getAllUserIds(): Promise<number[]> {
  const res = await pool.query(`SELECT id FROM bot_users`);
  return res.rows.map((row: { id: string | number }) => Number(row.id));
}
...
function checkRate(uid: number, isPremium: boolean): boolean {
  const limit = isPremium ? RATE_PREMIUM_PER_MIN : RATE_FREE_PER_MIN;
  const now = Date.now();
  const window = 60_000;
  const arr = (rateMap.get(uid) ?? []).filter((t) => now - t < window);
  if (arr.length >= limit) {
    rateMap.set(uid, arr);
    return false;
  }
  ...
  try {
    await pool.query(
      `INSERT INTO bot_usage (user_id, kind, model, input_tokens, output_tokens, cost_usd)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [uid ?? null, kind, model, inputTokens, outputTokens, cost],
    );
  } catch (e) {
...
const aiCall = async (sys: string, usr: string, tokens = 1000, meta: any = {}) => {
  const res = await textEngine.messages.create({
    model: TEXT_MODEL, max_tokens: tokens, system: sys,
    messages: [{ role: "user", content: usr }]
  });
  const out = (res.content[0] as any).text;
  await logUsage(meta.uid, meta.kind, out.length, tokens);
  return out;
}
...
async function composeWithHistory(
  ...
  user: string,
  max = 2000,
  meta: { uid?: number; kind: string } = { kind: "compose_history" },
): Promise<string> {
  ...
  await trackUsage(meta.uid, meta.kind, TEXT_MODEL, inT, outT, calcTextCost(inT, outT));
  return text;
}

async function renderImage(
  prompt: string,
  meta: { uid?: number; kind: string } = { kind: "image" },
): Promise<Buffer> {
  const res = await imageEngine.models.generateImages({
    model: IMAGE_MODEL,
    prompt,
    config: { numberOfImages: 1, outputMimeType: "image/png" },
  });
  await trackUsage(meta.uid, meta.kind, IMAGE_MODEL, 0, 0, PRICE_IMAGE);
...
async function describePhoto(
  ...
  system: string,
  instruction: string,
  meta: { uid?: number; kind: string } = { kind: "vision" },
): Promise<string> {
...
type ScrapingSource = "youtube" | "article" | "raw";



async function scrapeContent(url: string): Promise<any> {
  if (url.includes("youtube.com") || url.includes("youtu.be")) {
    return { text: "Simulated YouTube Transcript", source: "youtube", metadata: { url } };
  }
  return { text: "Simulated Web Article Content", source: "article", metadata: { url } };
}

const PROMPT_A_B_TEST = `Сгенерируй два варианта поста для A/B тестирования.
Вариант А: Эмоциональный, через сторителлинг.
Вариант Б: Прагматичный, через выгоду и цифры.
Верни оба варианта с разделителем ===.`;

const PROMPT_EMOJI_ENGINE = `Подбери идеальные эмодзи для текста, не перебарщивая. 
Используй только те, что усиливают смысл.`;

async function applyEmojiEngine(text: string): Promise<string> {
  return text + " ✨";
}

const logUsage =(uid: number, action: string, tokens: number) {
  await pool.query('INSERT INTO bot_usage (user_id, kind, input_tokens) VALUES ($1, $2, $3)', [uid, action, tokens]);
}

const PROMPT_REELS = `Ты — сценарист виральных Reels и Shorts. 
Создай сценарий, который удержит внимание.

Формат ответа:
🪝 ХУК (3 варианта первой фразы)
🎬 ВИЗУАЛ (описание кадров и движений)
🗣 ТЕКСТ (речь по секундам, живая и динамичная)
💡 СОВЕТ (как снять, чтобы залетело)

Никакого markdown. Пиши по делу.`;

const PROMPT_POST = `Expert Copywriter Mode.

- Стиль: живой, рваный ритм, без воды.
- Запрет: списки с эмодзи, клише "в мире", "сегодня мы".
- Фактура: только из контекста. Нет контекста - пиши из головы, но конкретно.
- Формат: хук -> мясо -> CTA. Без разметки.
...
const PROMPT_PLAN = `Ты — ведущий контент-стратег крупнейших медиа-холдингов. Твоя задача — создать план, который гарантирует рост охватов.

- Пиши как живой человек: используй разный ритм предложений, разговорные обороты, иногда задавай риторические вопросы.
- Никаких списков с эмодзи в каждом пункте. Эмодзи — только для акцентов (1-2 на пост).
- Избегай структуры "Заголовок - Список - Итог". Делай упор на связный текст и сторителлинг.
- Если есть входной контекст (BASE_KNOWLEDGE) — используй только его факты, не выдумывай лишнего.
Ты — контент-стратег. Составь контент-план на 7 дней для Telegram-канала под нишу пользователя.
...
const ACTIONS: Record<ActionKey, { ru: string; en: string }> = {
  ...
  reels: { ru: "🎬 Сценарий Reels", en: "🎬 Reels Script" },
  context: { ru: "📎 Добавить контекст", en: "📎 Add Context" },
  ab_test: { ru: "🅰️/🅱️ Тест поста", en: "🅰️/🅱️ A/B Test" },
  scrape: { ru: "🌐 Парсинг ссылки", en: "🌐 Link Scrape" },
};
...
function mainMenu(lang: Lang): Keyboard {
  return new Keyboard()
    ...
    .text(label("reels", lang)).text(label("context", lang)).row()
    .text(label("ab_test", lang)).text(label("scrape", lang)).row()
...
const STRINGS = {
  ru: {
    ...
    welcome: (name: string, status: string, uid: number) =>
...
async function loadOnboarding(uid: number): Promise<{
  lang: Lang | null;
...
async function saveLang(uid: number, lang: Lang): Promise<void> {
  await pool.query(
...
async function setCaptchaTarget(uid: number, target: string | null): Promise<void> {
...
async function markCaptchaPassed(uid: number): Promise<void> {
  await pool.query(
...
function paywallMessage(uid: number, lang: Lang): string {
  const t = STRINGS[lang];
  return `${s.paywallTitle}\n\n${s.paywallBody(FREE_QUOTA, OWNER_CONTACT, uid)}`;
}
...
async function cancelScheduled(uid: number, id: number): Promise<boolean> {
  const res = await pool.query(
    ...
    try {
      ...
      for (const row of res.rows) {
        try {
          ...
          try {
            const userLang = await pool.query('SELECT ui_lang FROM bot_users WHERE id = $1', [row.user_id]).then(r => res.rows[0]?.ui_lang || 'ru');
            await bot.api.sendMessage(
              Number(row.user_id),
              STRINGS[userLang as Lang].sendChannelFail(msg),
            );
...
export async function startBot(): Promise<void> {
  ...
  setInterval(() => {
    const now = Date.now();
    for (const [uid, times] of rateMap.entries()) {
      const filtered = times.filter(t => now - t < 60_000);
      if (filtered.length === 0) rateMap.delete(uid);
      else rateMap.set(uid, filtered);
    }
  }, 5 * 60 * 1000);
  ...
  bot.use(
    session({
      initial: (): SessionData => ({
        mode: null,
        brand: { id: "default", name: "Default" },
        brands: [],
        analytics: { totalGens: 0, lastUsed: new Date().toISOString() },
        preferences: { autoHashtags: true, toneStrictness: 5 },
        history: [],
        lang: "ru",
        onboarded: false,
        schedule: {},
  ...
  bot.command("reset", async (ctx) => {
    ...
    ctx.session.tempContext = undefined;
    const lang = ctx.session.lang;
  ...
  bot.command("grant", async (ctx) => {
    ...
    const until = await grantSubscription(uid, days, plan);
    await ctx.reply(
      `✅ Подписка \`${plan}\` для \`${uid}\` активна до ${until.toLocaleString("ru-RU")}.`,
      { parse_mode: "Markdown" },
    );
    try {
      await bot.api.sendMessage(
  ...
  bot.command("revoke", async (ctx) => {
    ...
    await ctx.reply(`❌ Подписка пользователя \`${uid}\` снята.`, {
      parse_mode: "Markdown",
    });
  });
  ...
  bot.command("reset_quota", async (ctx) => {
    ...
    await resetQuota(uid);
    await ctx.reply(`♻️ Пробный лимит пользователя \`${uid}\` сброшен.`, {
      parse_mode: "Markdown",
    });
  });
  ...
  bot.hears(labels("ab_test"), async (ctx) => {
    if (!(await ensureAccess(ctx))) return;
    ctx.session.mode = "ab_test";
    await ctx.reply(ctx.session.lang === "ru" ? "О чем написать два варианта поста?" : "What should I write two post variants about?");
  });

  bot.hears(labels("scrape"), async (ctx) => {
    if (!(await ensureAccess(ctx))) return;
    ctx.session.mode = "scrape";
    await ctx.reply(ctx.session.lang === "ru" ? "Пришли ссылку на статью или YouTube видео." : "Send a link to an article or YouTube video.");
  });

  bot.hears(labels("reels"), async (ctx) => {
    if (!(await ensureAccess(ctx))) return;
    ctx.session.mode = "reels_topic";
    await ctx.reply(ctx.session.lang === "ru" ? "О чем будет Reels? Опиши идею или тему." : "What is the Reels about? Describe the idea or topic.");
  });

  bot.hears(labels("context"), async (ctx) => {
    if (!(await ensureAccess(ctx))) return;
    ctx.session.mode = "context_input";
    await ctx.reply(ctx.session.lang === "ru" ? "Пришли текст, ссылку или данные, которые я должен использовать как базу для контента." : "Send text, link or data that I should use as a base for content.");
  });
  ...
  const status = await getUserStatus(ctx.from.id);
    ...
    if (data === "post_regen") {
      await ctx.reply(t.workingRegen);
      ...
      await logGeneration(uid, "post_regen", last.slice(0, 200), out);
      return;
    }
    if (data === "post_stronger") {
      await ctx.reply(t.workingStronger);
      const out = await compose(
        PROMPT_POST,
        `${brandContext(ctx.session.brand)}Сделай этот пост ещё сильнее: жёстче хук, конкретнее польза, мощнее CTA.\n\nТекст:\n${last}`,
        2000,
        { uid, kind: "post_stronger" },
      );
      ctx.session.lastPost = out;
      await ctx.reply(out, { reply_markup: postFollowupKeyboard(lang) });
      await logGeneration(uid, "post_stronger", last.slice(0, 200), out);
      return;
    }
    if (data === "post_shorter") {
      await ctx.reply(t.workingShorter);
      const out = await compose(
        PROMPT_POST,
        `Сократи пост до 400–600 знаков, сохранив суть и силу:\n\n${last}`,
        1200,
        { uid, kind: "post_shorter" },
      );
      ctx.session.lastPost = out;
      await ctx.reply(out, { reply_markup: postFollowupKeyboard(lang) });
      await logGeneration(uid, "post_shorter", last.slice(0, 200), out);
      return;
    }
    if (data === "post_longer") {
      await ctx.reply(t.workingLonger);
      const out = await compose(
        PROMPT_POST,
        `Расширь пост до 1500–2000 знаков, добавив примеры, цифры и конкретику:\n\n${last}`,
        2500,
        { uid, kind: "post_longer" },
      );
      ctx.session.lastPost = out;
      await ctx.reply(out, { reply_markup: postFollowupKeyboard(lang) });
      await logGeneration(uid, "post_longer", last.slice(0, 200), out);
      return;
    }
    if (data === "post_image") {
      await ctx.reply(t.workingImage);
      try {
        ...
        await logGeneration(uid, "post_image", last.slice(0, 200), promptImg);
      } catch (e) {
    ...
    if (data === "post_hashtags") {
      await ctx.reply(t.workingHashtags);
      const out = await compose(PROMPT_HASHTAGS, last, 800, {
        uid,
        kind: "post_hashtags",
      });
      await ctx.reply(out);
      await logGeneration(uid, "post_hashtags", last.slice(0, 200), out);
      return;
    }
    if (data === "post_repurpose") {
      await ctx.reply(t.workingRepurpose);
      const out = await compose(PROMPT_REPURPOSE, last, 3000, {
        uid,
        kind: "post_repurpose",
      });
      await ctx.reply(out);
      await logGeneration(uid, "post_repurpose", last.slice(0, 200), out);
      return;
    }
  });
  ...
  bot.on("message:photo", async (ctx) => {
    ...
    if (!largest) return ctx.reply(s.photoFail("No photo found"));
    try {
  ...
  bot.on("message:text", async (ctx) => {
    ...
    if (ctx.from && Object.keys(ctx.session.brand).length === 0) {
      ctx.session.brand = await loadBrand(ctx.from.id);
    }
    ...
    if (mode === "ab_test") {
      await ctx.replyWithChatAction("typing");
      const out = await compose(PROMPT_A_B_TEST, text, 3000, { uid, kind: "ab_test" });
      const [a, b] = out.split("===");
      await ctx.reply("Вариант А:\n" + a.trim());
      await ctx.reply("Вариант Б:\n" + b.trim(), { reply_markup: mainMenu(lang) });
      ctx.session.mode = null;
      return;
    }

    if (mode === "scrape") {
      await ctx.reply("⏳ Парсим контент...");
      const res = await scrapeContent(text);
      ctx.session.tempContext = res.text;
      await ctx.reply(`✅ Контент извлечен (${res.source}). Теперь он будет использоваться как база.`, { reply_markup: mainMenu(lang) });
      ctx.session.mode = null;
      return;
    }
    ...
    if (mode === "reels_topic") {
      await ctx.replyWithChatAction("typing");
      const contextStr = ctx.session.tempContext ? `\nBASE_KNOWLEDGE:\n${ctx.session.tempContext}\n` : "";
      const out = await compose(
        PROMPT_REELS,
        `${brandContext(ctx.session.brand)}${contextStr}Тема: ${text}`,
        2000,
        { uid, kind: "reels" }
      );
      await ctx.reply(out, { reply_markup: mainMenu(lang) });
      await logGeneration(uid, "reels", text, out);
      if (uid) await consumeQuota(uid);
      ctx.session.mode = null;
      return;
    }

    if (mode === "context_input") {
      ctx.session.tempContext = text;
      ctx.session.mode = null;
      await ctx.reply(lang === "ru" ? "✅ Контекст сохранен! Теперь при генерации постов или сценариев я буду опираться на эти данные." : "✅ Context saved! I will use this data when generating posts or scripts.");
      return;
    }
    ...
    if (mode === "post_topic") {
      ...
      const out = await compose(
        PROMPT_POST,
        `${brandContext(ctx.session.brand)}${ctx.session.tempContext ? `\nBASE_KNOWLEDGE:\n${ctx.session.tempContext}\n` : ""}Тема: ${text}`,
        2000,
        { uid, kind: "post" },
      );
      ctx.session.lastPost = out;
      ...
      await logGeneration(uid, "post", text, out);
      if (uid) await consumeQuota(uid);
      return;
    }
    ...
    if (mode === "post_followup") {
      ...
      ctx.session.lastPost = out;
      await ctx.reply(out, { reply_markup: postFollowupKeyboard(lang) });
      await logGeneration(uid, "post_edit", text, out);
      if (uid) await consumeQuota(uid);
      return;
    }
    ...
    if (mode === "hook_topic") {
      ...
      await ctx.reply(out, { reply_markup: mainMenu(lang) });
      await logGeneration(uid, "hooks", text, out);
      if (uid) await consumeQuota(uid);
      ctx.session.mode = null;
      return;
    }
    ...
    if (mode === "headline_topic") {
      ...
      await ctx.reply(out, { reply_markup: mainMenu(lang) });
      await logGeneration(uid, "headlines", text, out);
      if (uid) await consumeQuota(uid);
      ctx.session.mode = null;
      return;
    }
    ...
    if (mode === "image_prompt") {
      await ctx.reply(t.workingGenImage);
      try {
        ...
        await logGeneration(uid, "image", text, englishPrompt);
        if (uid) await consumeQuota(uid);
      } catch (e) {
        await ctx.reply(s.imageFail((e as Error).message));
      }
    ...
    if (mode === "analyze_text") {
      ...
      await ctx.reply(out, { reply_markup: mainMenu(lang) });
      await logGeneration(uid, "analyze", text, out);
      if (uid) await consumeQuota(uid);
      ctx.session.mode = null;
      return;
    }
    ...
    if (mode === "repurpose_text") {
      ...
      await ctx.reply(out, { reply_markup: mainMenu(lang) });
      await logGeneration(uid, "repurpose", text, out);
      if (uid) await consumeQuota(uid);
      ctx.session.mode = null;
      return;
    }
    ...
    if (mode === "hashtag_topic") {
      ...
      await ctx.reply(out, { reply_markup: mainMenu(lang) });
      await logGeneration(uid, "hashtags", text, out);
      if (uid) await consumeQuota(uid);
      ctx.session.mode = null;
      return;
    }
    ...
    if (mode === "rewrite_text") {
      ...
      await ctx.reply(out, { reply_markup: mainMenu(lang) });
      await logGeneration(uid, "rewrite", text, out);
      if (uid) await consumeQuota(uid);
      ctx.session.mode = null;
      return;
    }
...
async function cleanupOldLogs() {
  ...
  await pool.query('DELETE FROM bot_usage WHERE created_at < $1', [cutoff]);
}

export function validateConfig() {
  const required = ['TELEGRAM_BOT_TOKEN', 'AI_INTEGRATIONS_ANTHROPIC_API_KEY'];
...
async function getTopUsers() {
  const res = await pool.query('SELECT user_id, COUNT(*) as count FROM bot_generations GROUP BY user_id ORDER BY count DESC LIMIT 10');
  return res.rows;
}
